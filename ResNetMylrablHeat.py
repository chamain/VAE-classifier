...
from ops import resblock,bottle_resblock,get_residual_layer,conv,batch_norm,global_avg_pooling,fully_conneted,relu,classification_loss,entropyLossHeat
...
class ResNet(object):
    def __init__(self, sess, args):
        ...
        self.__log = open('logbeta1lr_6n2beta4heat.txt','w')
    ...
    def encoder(self, x, is_training=True, reuse=False):
        with tf.variable_scope("encoder", reuse=reuse):
            ...
            heatmap = _get_heatmap3D(bottleneck=x)
            x = _mask_with_heatmap(x, heatmap)
            x = transpose_NCHW_to_NHWC(x)
            print('******************encoder break after heat map*********** resblock2_:',x.shape)
            qout = self.quantizer(x,is_training=True, reuse=False, data_format = 'NCHW')
    ...
    def build_model(self):
        ...
        """ Model """
        self.train_logits,self.train_bc, self.train_bpp,self.train_heatmap = self.network(self.train_inptus)
        ...
        self.test_logits,self.test_bc,self.test_bpp,self.test_heatmap = self.network(self.test_inptus, is_training=False, reuse=True)
        ...
        self.train_Hreal, self.train_Hmask, self.train_H = entropyLossHeat(self.train_bc,self.H_target,self.train_heatmap)
        self.train_entropy_loss = self.beta*self.train_H
        self.test_loss = self.test_closs + reg_loss
        self.test_Hreal, self.test_Hmask, self.test_H = entropyLossHeat(self.test_bc,self.H_target,self.test_heatmap)
        self.test_entropy_loss = self.beta*self.test_H
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.epoch):
            ...
            for idx in tqdm(range(start_batch_id, self.iteration)):
...
# =============================================================================
        ...
        batchT = self.ip_train.get_batch()
        acc = 0
        bpp = 0
        Rbpp = 0
        Mbpp = 0
        ...
        for idx in range(self.tsiteration):
            ...
            Rbpp += test_Rbpp
            Mbpp += test_Mbpp
            #print("iter:{}".format(idx), " test_accuracy: {}".format(test_accuracy)," bpp: {}".format(test_bpp)," Rbpp: {}".format(test_Rbpp)," Mbpp: {}".format(test_Mbpp))
        print("test_accuracy: {}".format(acc/self.tsiteration)," bpp: {}".format(bpp/self.tsiteration)," Rbpp: {}".format(Rbpp/self.tsiteration)," Mbpp: {}".format(Mbpp/self.tsiteration))
