...
def entropyLossHeat(bc,H_target,heatmap):
       ...
       # H_soft = tf.reduce_mean(bc, name='H_softl')
        ...
        return H_real,H_mask,tf.maximum(H_soft - H_target, 0)
    
def entropyLossHeat2(bc,H_target,heatmap):
        
        bc_mask = bc[:,:32,] * heatmap
        H_real = tf.reduce_mean(bc[:,:32,], name='H_real')
        H_mask = tf.reduce_mean(bc_mask, name='H_mask')
        #H_soft = 0.5 * (H_mask + H_real)
       # H_soft = tf.reduce_mean(bc, name='H_softl')
        ...
        return H_real/2,H_mask/2,tf.maximum(H_mask- H_target, 0)
