...
class ImageNetTFExampleInput(object):
  ...
  def dataset_parser(self, value):
    ...
    label = tf.cast(
        tf.reshape(parsed['image/class/label'], shape=[]), dtype=tf.int64)
  ...
  def input_fn(self, params):
    ...
    ######################################################################################To make chalnnel last
# =============================================================================
#     dataset = dataset.map(
#           lambda images, labels: (tf.transpose(images, [0, 3, 1,2]), labels),
#           num_parallel_calls=self.num_parallel_calls)
# =============================================================================
    ...
    #dataset = tf.data.Dataset.interleave(dataset)
    return dataset
...
class ImageNetInput(ImageNetTFExampleInput):
  ...
  def make_source_dataset(self, index, num_hosts):
    ...
    def fetch_dataset(filename):
      buffer_size = 8 * 1024* 1024  # 8 MiB per file ###########################changed
    ...
    if self.cache:
      dataset = dataset.cache().apply(
          tf.data.experimental.shuffle_and_repeat(10)) ##########changes
    else:
      dataset = dataset.shuffle(1024) #changed
    return dataset
